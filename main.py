...
# Font colors
DICE_COLOR = (0, 0, 139)      # Dark blue for dice text
MOVE_COLOR = (0, 100, 0)      # Dark green for move text
...
hiss_sound = pygame.mixer.Sound("assets/sounds/snake_hiss.wav")
...
while running:
    ...
    if rolling:
        if frame < len(DICE_IMAGES):
            ...
            old_position = player.position  # Save the old position before moving
            player.move(steps)

            # Pass old_position to apply_special_tiles (this fixes your error!)
            move_message, move_type = apply_special_tiles(player, steps, old_position)

            # Check win AFTER applying special tile effects!
