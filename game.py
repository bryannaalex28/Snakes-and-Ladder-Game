...
def apply_special_tiles(player, dice_roll):
    ...
    if final_position in special_tiles["teleport"]:
        ...
        return f"Teleported from {final_position} to {new_position}!"

    # Extra roll
    if final_position in special_tiles["extra_roll"]:
        return f"Moved from {old_position} to {final_position}. Extra roll!"

    # Reverse snake 
    if final_position in special_tiles["reverse_snake"]:
        new_position = final_position + 5
        player.position = min(new_position, 99)
        return f"Reverse snake at {final_position}! Jumped forward to {player.position}!"

    # Ladders
    if final_position in ladders:
        new_position = ladders[final_position]
        player.position = new_position
        return f"Climbed a ladder from {final_position} to {new_position}!"

    # Snakes
    if final_position in snakes:
        new_position = snakes[final_position]
        player.position = new_position
        return f"Got bitten by a snake from {final_position} to {new_position}!"

    # Normal move
    return f"Moved from {old_position} to {final_position}"
