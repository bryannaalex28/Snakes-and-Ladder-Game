...
def apply_special_tiles(player, dice_roll, old_position):
    ...
    if final_position in special_tiles["teleport"]:
        ...
        return f"Teleported from {final_position + 1} to {new_position + 1}!", "teleport"

    # Extra roll
    if final_position in special_tiles["extra_roll"]:
        return f"Moved from {old_position + 1} to {final_position + 1}. Extra roll!", "extra_roll"

    # Reverse snake
    if final_position in special_tiles["reverse_snake"]:
        new_position = final_position + 5
        player.position = min(new_position, 99)
        return f"Reverse snake at {final_position + 1}! Jumped forward to {player.position + 1}!", "reverse_snake"

    # Ladders
    if final_position in ladders:
        new_position = ladders[final_position]
        player.position = new_position
        return f"Climbed a ladder from {final_position + 1} to {new_position + 1}!", "ladder"

    # Snakes
    if final_position in snakes:
        new_position = snakes[final_position]
        player.position = new_position
        return f"Got bitten by a snake from {final_position + 1} to {new_position + 1}!", "snake"

    # Normal move
    return f"Moved from {old_position + 1} to {final_position + 1}", "move"
